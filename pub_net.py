...
def get_post(url,sleept=5,header=None,cookies={},method="GET",data={}):#url 失败后等待时间 是否需要重编码 headers cookies
    _headers = header if header else {'User-Agent': ua()}
    while 1:
        der=0
        try:
            r=requests.request(url=url,headers=_headers,cookies=cookies,data=data,method=method)
            r.raise_for_status()
